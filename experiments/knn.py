...
f1 = f"F1 score score for KNN Classifier is: {(round(f1_score(y_test, knn_pred, average='macro'),4))}"
prec_score = f"Precision score for KNN Classifier is: {(round(precision_score(y_test, knn_pred, average='macro'),4))}"
acc_score = f"Accuracy score for KNN Classifier is: {(round(accuracy_score(y_test, knn_pred),4))}"
crs_val_score = f"Mean of cross val score for KNN Classifier is: {(round(np.mean(cross_val_score(neigh, X, y, cv=5)),4))}"
...
for (train_index, test_index) in rskf.split(X, y):
    ...
    support_matrix = neigh.predict(x_test_2)
